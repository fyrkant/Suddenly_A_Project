...
export let FilterMenu = (props) => {
  ...
  const orderMenuItems = map(orders, (order, key) => {
    return (isEqual(props.order, order)
      ...
      : <MenuItem key={key} onClick={props.orderChangeHandler.bind(this, order)}>
      {orderMenuString[key]}
      </MenuItem>
    )
  })
  ...
  return (
    <div>
      <span id='filter-menu'><IconButton name='more_vert' /> {currentFilter}</span>
