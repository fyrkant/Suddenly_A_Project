  ...
  return (
    <GoogleMapLoader
      ...
      googleMapElement={
        ...
        >
          {map(props.messages, message => <Marker
            key={message.id}
            icon={pinUrl + priorityColors[message.priority]}
            opacity={props.selected.id === null ? 1 : props.selected.id === message.id ? 1 : 0.2}
            ...
            {props.selected.id === message.id
              ? <InfoWindow onCloseclick={props.selectMessage.bind(this, message.id)}>
                <DetailedMessage message={props.selected} />
              </InfoWindow>
             : ''}
          </Marker>
            )}
