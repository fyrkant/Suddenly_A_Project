    ...
    <Card shadow={0} style={{width: '250px', margin: '0', padding: '0'}} onClick={props.onClick}>
      <CardText>
        ...
        <p>
          ...
          {m(props.message.createddate).format('Do MMMM YYYY, LT')}
        </p>
        <p style={{fontColor: priorityColors[props.message.priority]}}>
          <strong>Prioritet:</strong>
          {prioStr}
        </p>
        {
          props.message.subcategory
           ? <p>
        ...
        {
          props.message.exactlocation
          ? <p>
            <strong>Exakt plats:</strong>
            {props.message.exactlocation}
          </p> : ''}
      </CardText>
    </Card>
